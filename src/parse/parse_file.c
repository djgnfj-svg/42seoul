...
void	ft_number_elements(char *line, t_rt *rt)
{
...
void	ft_load_element(char *line, t_rt *rt)
{
	if (ft_strncmp(line, "R ", 2) == 0)
		parse_res(line, rt);
	else if (ft_strncmp(line, "A ", 2) == 0)
...
void	ft_set_elements(t_rt *rt)
{
	if (rt->index[0] != 1 || rt->index[1] != 1)
		ft_error_handler(BAD_FORMAT);
	...
	if (!(rt->cylinder = malloc((rt->index[7] + 1)
					* sizeof(t_cylinder *))))
		ft_error_handler(MEM_ALLOC);
	rt->cylinder[rt->index[7]] = NULL;
	if (!(rt->triangle = malloc((rt->index[8] + 1)
					* sizeof(t_triangle *))))
		ft_error_handler(MEM_ALLOC);
